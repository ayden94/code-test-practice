...
type AllCombinations<S extends string, R extends [string, string] = never>
  = S extends `${infer F}${infer T}`
    ? AllCombinations<T, [F, ''] | ['', F] | R | (R extends R ? [`${F}${R[0]}`, R[1]] | [`${R[0]}${F}`, R[1]] | [R[0], `${F}${R[1]}`] | [R[0], `${R[1]}${F}`] : never)>
  : '' | (R extends [string, string] ? `${R[0]}${R[1]}` : never)
