...
/**
  ...
  K는 foo | string인데 이게 각각 분리돼서 처리되나?
  그럼 유니언 타입에 대한 as는 각 타입에 대해 처리된다는 뜻인데?

  K = foo | string
  P = string | symbol | number

  P extends K ? never : K extends P ? K : never
...
(foo | never | never) | (never | never | never)
(foo | never)
(foo)
...
type T = number extends "foo" ? true : false
